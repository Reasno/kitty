...
- if: true
  then:
    foo: quz
`)
	dynConf, err := NewRuleEngine(WithClient(client), Rule("wfdj-luckBox-dev"), WithDMPAddr("dmp-grpc.dev.tagtic.cn:443"), WithEnv(kconf.Env("local")))
	if err != nil {
		t.Fatal(err)
	}
	...
	if conf.String("foo") != "quz" {
		t.Fatalf("want %s, got %s", "quz", conf.String("foo"))
	}
}
