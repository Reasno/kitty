...
func MakeHTTPHandler(endpoints Endpoints, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("POST").Path("/bind-ad").Handler(httptransport.NewServer(
		endpoints.BindAdEndpoint,
		DecodeHTTPBindAdZeroRequest,
		EncodeHTTPGenericResponse,
		serverOptions...,
	))
...
// DecodeHTTPBindAdZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded bindad request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPBindAdZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.UserBindAdRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
